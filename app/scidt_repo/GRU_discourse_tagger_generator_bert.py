...
from keras.layers import Input, LSTM, Dense, Dropout, TimeDistributed, Bidirectional, GRU
...
class PassageTagger(object):
    ...
    def fit_model(self, train_generator, validation_generator, reg=0):
        ...
        if bidirectional:
            x = Bidirectional(GRU(input_shape=(self.maxseqlen,word_proj_dim), units=lstm_dim,
            ...
            x = GRU(input_shape=(self.maxseqlen,word_proj_dim), units=lstm_dim, return_sequences=True,
                            kernel_regularizer=l2(reg),
                            recurrent_regularizer=l2(reg), 
                            bias_regularizer=l2(reg))(x)
            x = Dropout(lstm_dropout)(x) 
        ...
        for lr_fraction in lr_fractions:
            ...
            tagger.fit_generator(train_generator, validation_data=validation_generator, epochs=epoch, callbacks=[early_stopping, checkpoint])
